...
maincmd.setCallback(function(cmd,origin,output,results){
	if (results.accept == "accept" || results.accept == "a") {//指令接受
		if (individualpreferences.get(origin.player.uuid).active) {
			tpa(origin.player);
		}
		else {
	...
	//tpahere
		...
		/*if (individualpreferences.get(origin.player.uuid).active) {//tpahere to
			limits: {
				if (conf.get("frequency_limit").active && toooften(origin.player) && conf.get("frequency_limit").limit_price > 0) {//频繁且付费
					//频繁后可以付费
					//特殊位置，单独写一份
					origin.player.sendModalForm("发送tpa请求过于频繁", `发送tpa请求过于频繁！暂时您需要花费${conf.get("frequency_limit").limit_price + conf.get("economy").price}${conf.get("economy").name}才能继续传送。`, "继续", "取消", function (player, result) {
						if (result) {//点击确定
							if (economy.get(player) < conf.get("economy").price + conf.get("frequency_limit").limit_price) {
								player.tell("您的余额不足");
							} else {
								//执行tpahere
								whethertpato(player,results.target,results.to=="to","tpahere");
							}
						}
					});
					break limits;
				}
				else if (conf.get("frequency_limit").active && toooften(origin.player) && conf.get("frequency_limit").limit_price <= 0) {
					origin.player.tell("发送tpa请求过于频繁，请稍后再试");
					break limits;
				}
				if (economy.get(origin.player) < conf.get("economy").price && conf.get("economy").price > 0) {
					origin.player.tell(`您的余额不足，本服务器中tpa传送需花费${conf.get("economy").price}${conf.get("economy").name}`);
					break limits;
				}
				//执行tpahere
				whethertpato(origin.player,results.target,results.to=="to","tpahere");
			}
		}
		else {
			origin.player.tell("您未开启tpa。输入/tpa switch来开启。")
	...
	//tpa开关
		...
		let write = individualpreferences.get(origin.player.uuid);
		write.active = !write.active
		individualpreferences.set(origin.player.uuid, write);
		if (write.active) { origin.player.tell("您已经开启了tpa功能。输入/tpa preferences来调整偏好设置。"); }
...
mc.listen("onJoin",(player)=>{
	individualpreferences.init(player.uuid, { name: player.name, active: conf.get("default_preferences").active, requestavailable: conf.get("default_preferences").requestavailable, acceptmode: conf.get("default_preferences").acceptmode })
})
...
function tpa(player){//只有从缓存中读取请求时才会调用这个函数，缓存中标明了是tpa还是tpahere，所以即使是tpahere，也会用这个函数传送
	...
	for(i=0;i<cachedrequests.length;i++){
		...
		if(cachedrequests[i].target.uuid==player.uuid&&new Date().getTime()-cachedrequests[i].time<=individualpreferences.get(cachedrequests[i].origin.uuid).requestavailable){
...
function tpaform(player,type){
	...
	if(onlineplayers.length==0){player.tell("现在只有您自己在线。");}
		...
		player.sendForm(fm,function(player,id){
			if(id!=null){
				if(individualpreferences.get(onlineplayers[id].uuid).active){
...
function tpask(player,origin,type){
	switch(individualpreferences.get(player.uuid).acceptmode){
		case 1:{//弹窗提醒
		...
		case 2:{//指令接受
			if(type=="tpa"){
				...
				origin.tell(`由于${player.name}未设置弹窗提醒，对方输入指令同意前，您的请求将有效${individualpreferences.get(origin.uuid).requestavailable/1000}秒。你可通过/tpa preference调整有效时间。`)
...
function individualpreferencesform(player){
	...
	fm.addLabel("（此功能未开发完成）如果关闭此开关，您将立即拒绝任何tpa请求，且/tpa指令除/tpa preferences外均禁用。您可通过输入/tpa preferences再次启用tpa。")
	fm.addSwitch("tpa开关",individualpreferences.get(player.uuid).active);
	fm.addDropdown("接收到tpa请求时",mode,individualpreferences.get(player.uuid).acceptmode)
	fm.addInput("tpa请求有效时间/秒"," ",(individualpreferences.get(player.uuid).requestavailable/1000).toString())//
	player.sendForm(fm,function (player,data){
		if(data!=null){
			preferences=individualpreferences.get(player.uuid)
			...
			if(Number.isSafeInteger(Number(data[3]))){
				...
				player.tell(`您输入的tpa请求有效时间不完全是数字。已将您的tpa请求有效时间设置为120秒。`)
				preferences.requestavailable=120000
				
			}
			individualpreferences.set(player.uuid,preferences);
	...
	if(type=="tpa"){
		...
		target.sendForm(fm,function(player,id){
			if (id == 0) {
				...
				target.tell(`您已成功缓存了此请求。之后的${individualpreferences.get(origin.uuid).requestavailable/1000}内，输入/tpa accept来接受此请求，输入/tpa deny来拒绝此请求`)
			}
			else{
...
function whethertpa(origin,targetarr,to,type){
	if (individualpreferences.get(origin.uuid).active) {//tpa
		limits: {
...
/*function tpahereform(player){
	...
	if(onlineplayers.length==0){player.tell("现在只有您自己在线。");}
		...
		player.sendForm(fm,function(player,id){
			if(id!=null){
				if(individualpreferences.get(onlineplayers[id].uuid).active){
					tpask(onlineplayers[id],player,"tpahere");
				}
				else{
					player.tell(`${onlineplayers[id].name}未开启tpa。`)
				}				
